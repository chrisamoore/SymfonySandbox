...
stage { 'vhost_tools': }
stage { 'frontend': }
stage { 'frontend_view_bundle': }
stage { 'bounce': }

Stage['main'] -> Stage['nginx_setup'] -> Stage['postsetup'] -> Stage['frontend'] -> Stage['frontend_view_bundle'] -> Stage['database'] -> Stage['vhost_tools'] -> Stage['bounce']
...
class nginx_setup {
    ...
    exec { 'conf_files':
        cwd     => "/etc/nginx/conf.d/",
        command => "cp -ir /vagrant/files/nginx/symfony.conf ./symfony.conf",
        path => "/usr/bin:/usr/sbin:/bin:/usr/local/bin",
        require => Exec['ini_files']
    }
...
class setup {
    ...
    exec{ 'assetic':
        ...
        command => 'php app/console assets:install && php app/console assetic:install',
        path => ['/usr/local/bin','/usr/bin', '/bin', '/sbin'],
        returns => [ 0, 1, 2, 255],
        user => 'vagrant',
    ...
    file { "/vagrant/Symfony/app/cache":
        owner  => "vagrant",
        mode   => 777,
    }

    file { "/vagrant/Symfony/app/logs":
        owner  => "vagrant",
        mode   => 777,
    }
}
...
class database {
    # exec { "clear_db":
    #   cwd => '/vagrant/Symfony',
    #   command => 'php app/console doctrine:schema:drop',
    ...
    exec { "setup_db":
        ...
        logoutput => true,
        returns => [ 0, 1, 255 ],
    # require => Exec['clear_db'],
    }

    # exec { "load_fixtures":
    #   cwd => '/vagrant/Symfony',
    #   command => 'php app/console doctrine:fixtures:load --append',
    #   path => ['/usr/bin', '/bin', '/sbin'],
    #   logoutput => true,
    #   returns => [ 0, 1, 255 ],
...
#    exec {"PSR2":
#        command => "/vagrant/Symfony/bin/phpcs --config-set default_standard PSR2",
#        returns => [ 0, 1, 255 ]
#    }
}

class frontend {
	exec { "redis_commander_install":
        command => "npm install -g redis-commander",
        path => "/usr/bin:/usr/sbin:/bin:/usr/local/bin",
        returns => [ 0, 1, 255 ],
    }

    exec { "node_stuff_install":
        command => "npm install -g grunt grunt-cli bower",
        path => "/usr/bin:/usr/sbin:/bin:/usr/local/bin",
        returns => [ 0, 1, 255 ],
    }
}

class frontend_view_bundle {

    # exec { "run_grunt":
    #	cwd => "/vagrant/Symfony/vendor/ue-software/view-bundle/src/Uecode/Bundle/ViewBundle/Resources/private",
    #	command => "screen grunt -d -m",
    #	returns => [0, 1, 255],
    #	require => [Exec['npm_install_things'], Exec['bower_install_things']],
    # }
}
...
class{ 'aws': stage => vhost_tools }
class{ 'setup': stage => postsetup }
class{ 'database': stage => database }
class{ 'vhost_tools': stage => vhost_tools }
class{ 'frontend': stage => frontend }
class{ 'frontend_view_bundle': stage => frontend_view_bundle }
